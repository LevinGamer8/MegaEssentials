...
public class deleteWarp implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (!(p.hasPermission("megacraft.command.deletewarp"))) {
            p.sendMessage(MegaEssentials.Prefix + MegaEssentials.noPerms);
            return true;
        }

        if (!(args.length == 1)) {
            p.sendMessage(MegaEssentials.Prefix + "§4Nutzung§7: §b/deletewarp §7[§bNAME§7]");
            return true;
        }

        PlayerData pd = new PlayerData(p.getName());
        pd.removeWarp(args[0], p);
        p.sendMessage(MegaEssentials.Prefix + "§aDu hast den §bWarp " + args[0] + " §aerfolgreich §cgelöscht§7.");
